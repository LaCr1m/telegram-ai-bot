...
@_web_app.get("/")
async def health():
    return {"status": "ok", "bot": "J.A.R.V.I.S.", "time": now_kyiv().isoformat()}
...
def get_user_memory(user_id: int) -> dict:
    with _db() as con:
        with con.cursor() as cur:
            cur.execute("SELECT data FROM memory WHERE user_id=%s", (user_id,))
            row = cur.fetchone()
...
def update_user_memory(user_id: int, data: dict) -> None:
    ...
    with _db() as con:
        with con.cursor() as cur:
            cur.execute(
                "INSERT INTO memory(user_id,data) VALUES(%s,%s) "
...
def load_history_db(user_id: int) -> list:
    with _db() as con:
        with con.cursor() as cur:
            cur.execute("SELECT messages FROM history WHERE user_id=%s", (user_id,))
            row = cur.fetchone()
...
def save_history_db(user_id: int, messages: list) -> None:
    with _db() as con:
        with con.cursor() as cur:
            cur.execute(
                "INSERT INTO history(user_id,messages) VALUES(%s,%s) "
...
def get_user_tasks(user_id: int) -> list:
    with _db() as con:
        with con.cursor() as cur:
            cur.execute("SELECT data FROM tasks WHERE user_id=%s", (user_id,))
            row = cur.fetchone()
    return row["data"] if row else []
...
def set_user_tasks(user_id: int, tasks: list) -> None:
    with _db() as con:
        with con.cursor() as cur:
            cur.execute(
                "INSERT INTO tasks(user_id,data) VALUES(%s,%s) "
                "ON CONFLICT(user_id) DO UPDATE SET data=EXCLUDED.data",
...
def load_reminders() -> list:
    with _db() as con:
        with con.cursor() as cur:
            cur.execute("SELECT id, chat_id, text, fire_at FROM reminders")
...
def add_reminder(chat_id: int, text: str, fire_at: datetime) -> int:
    with _db() as con:
        with con.cursor() as cur:
            cur.execute(
                "INSERT INTO reminders(chat_id,text,fire_at) VALUES(%s,%s,%s) RETURNING id",
...
def save_session_log(user_id: int, summary: str, mood: str = "") -> None:
    with _db() as con:
        with con.cursor() as cur:
            cur.execute(
                "INSERT INTO session_log(user_id,created_at,summary,mood) VALUES(%s,%s,%s,%s)",
                (user_id, now_kyiv().isoformat(), summary[:500], mood),
            )
            cur.execute(
                "DELETE FROM session_log WHERE user_id=%s AND id NOT IN "
                "(SELECT id FROM session_log WHERE user_id=%s ORDER BY id DESC LIMIT 30)",
                (user_id, user_id),
            )
        con.commit()
...
async def detect_intent_ai(text: str) -> str:
    result = await call_ai([{"role": "user", "content": (
        ...
        f"Повідомлення: '{text}'\n\n"
        "НАМІРИ:\n"
        "• image — намалювати, згенерувати, створити зображення/фото/картинку\n"
        "• reminder — нагадати про щось (ОБОВ'ЯЗКОВО має бути час/дата)\n"
        "• search — знайти актуальну інформацію, новини, погоду, ціни\n"
        "• translate — перекласти текст\n"
        "• summarize — підсумувати статтю або текст\n"
        "• generate — написати/створити текст: резюме, лист, пост\n"
        "• edit — відредагувати або покращити існуючий текст\n"
        "• recipe — знайти рецепт або що приготувати\n"
        "• task — додати, видалити або показати задачу\n"
        "• chat — все інше\n\n"
        "Повертай ТІЛЬКИ одне слово без пояснень."
    )}])
...
async def transcribe_voice(audio_bytes: bytes) -> str:
    if not GROQ_API_KEY:
        raise RuntimeError("GROQ_API_KEY required for voice transcription")
...
async def _build_image_prompt(user_request: str, ctx_desc: str = "") -> str:
    if ctx_desc:
        ...
        instruction = (
            ...
            f"Return ONLY the English prompt.\n\nRequest: {user_request}"
        )
    ...
    log.info("Image prompt: %s", translation)
    return translation
...
async def generate_image(prompt: str) -> bytes:
    ...
    for attempt in range(3):
        try:
            ...
            last_error = e
            log.warning("generate_image attempt %d: %s", attempt + 1, e)
...
async def analyze_video(video_bytes: bytes, caption: str, user_id: int = 0) -> str:
    ...
    try:
        ...
        log.warning("analyze_video audio: %s", e)
    try:
        ...
        log.warning("analyze_video frames: %s", e)
    if not results:
...
def _detect_search_type(query: str) -> str:
    q = query.lower()
    if any(w in q for w in ["новини","новина","подія","що сталось","що відбулось"]):
        return "news"
    if any(w in q for w in ["погода","температура","дощ","сніг"]):
        return "weather"
    ...
    for i, chunk in enumerate(chunks):
        ...
        try:
            ...
            try:
                ...
                log.error("_send_or_edit failed: %s", e)
...
async def _dispatch_intent(
    ...
    if intent == "search":
        ...
        try:
            ...
            await msg.edit_text(f"{prefix}❌ Помилка пошуку.")
        return True
...
async def forget_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with _db() as con:
        with con.cursor() as cur:
            cur.execute("DELETE FROM memory WHERE user_id=%s", (uid,))
        con.commit()
...
async def handle_video(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await msg.edit_text("❌ Помилка аналізу відео.")
...
async def handle_document(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await msg.edit_text("❌ Помилка обробки документу.")

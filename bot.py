...
SYSTEM_PROMPT = {
    ...
    "content": (
        ...
        "Спілкуйся природно і дружньо — без офіційних зворотів. "
...
_PRACTICAL_MARKERS = ["порадь","поради","що робити","як","де","коли","хто","знайди","покажи","розкажи","поясни","допоможи","чому","навіщо","скільки","який","яка","яке","пам'ятаєш","ти знаєш"]
...
async def _compress_history(user_id: int) -> None:
    ...
    try:
        ...
        log.warning("_compress_history summary failed: %s", e)
        return
...
def detect_intent_local(text: str) -> str | None:
    ...
    for intent, keywords, weight in kw_map:
        for kw in keywords:
            if kw in t:
                scores[intent] += weight

    # Edit/generate потребують реального тексту після ключового слова (>5 слів після тригера)
    for intent in ("edit", "generate"):
        if scores[intent] > 0:
            has_content = any(
                len(t[t.index(kw) + len(kw):].strip().split()) > 5
                for kw in (EDIT_KEYWORDS if intent == "edit" else GENERATE_KEYWORDS)
                if kw in t
            )
            if not has_content:
                scores[intent] = 0
    ...
    # Reminder only with explicit time marker
...
def _escape_v2(text: str) -> str:
    """Escape спецсимволів для MarkdownV2, крім вже розмічених місць."""
